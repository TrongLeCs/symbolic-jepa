...
# Đảm bảo module dưới đây có hàm: build_ldp_links_tokenlevel(expr, tokenizer, project_mode="first"| "all")
from parser.ldp_links_api import build_ldp_links_tokenlevel


class LDPLinksDatasetBuilderT5:
    """
    Đọc danh sách record {topic, nl, fol} và đóng gói .npz cho nhiệm vụ LDP links ở MỨC TOKENIZER (T5 pieces):
      - topic_ids: (N,) int64
      ...
      - ldp_links: (N,) object[np.ndarray(L_i, L_i)]  # ma trận LDP T5 (int8), -1 mask cho ignored
      - ldp_edges: (N,) object[np.ndarray(E_i,2)]     # danh sách cạnh (src,dst) với value==1
    ...
    def _process_one(self, rec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        out = build_ldp_links_tokenlevel(
        ...
        mat_t5: np.ndarray = out["ldp_links_t5"]  # (1, L, L)
        ...
        return {
            ...
            "ldp_links": mat_nn.astype(np.int8, copy=False),
            "ldp_edges": edges_idx,  # (E,2)
    ...
    def build_and_save(
        ...
        *,
        ...
        ldp_list: List[np.ndarray] = []
        ldp_edges_list: List[np.ndarray] = []
        ...
        txt_buf = io.StringIO()
        txt_buf.write(f"# LDP Links (T5 level) dump for {output_npz_path.name}\n")
        ...
        for ridx, ex in enumerate(processed):
            ...
            tokens = ex["tokens"]
            types = ex["types"]
            mat = ex["ldp_links"]  # (L,L) int8
            edges_idx = ex["ldp_edges"]  # (E,2)
            ...
            ldp_list.append(mat)
            ldp_edges_list.append(edges_idx)
            ...
            # Ma trận kề
            L = mat.shape[0]
            txt_buf.write(f"ldp_links shape: {mat.shape}\n")
            PRINT_MATRIX = True
            if PRINT_MATRIX:
                write_matrix_rows(
                    mat,
        ...
        np.savez(
            ...
            ldp_links=np.asarray(ldp_list, dtype=object),
            ldp_edges=np.asarray(ldp_edges_list, dtype=object),
...
def load_packed_ldp_t5(npz_path: str | Path) -> Dict[str, Any]:
    """
    Tiện ích load .npz LDP ở mức T5.
    """
    ...
    with np.load(npz_path, allow_pickle=True) as z:
        return {
            ...
            "ldp_links": z["ldp_links"],  # (N,) object[np.ndarray(L_i,L_i)]
            "ldp_edges": z["ldp_edges"],  # (N,) object[np.ndarray(E_i,2)]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Build LDP-links dataset (.npz) ở mức T5 pieces từ JSON"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="data/samples_ldp_links.npz",
        help="Đường dẫn file .npz sẽ được tạo",
    )
    parser.add_argument(
        "--error_log",
        default="data/samples_error_log.jsonl",
    ...
    builder = LDPLinksDatasetBuilderT5(tokenizer=tokenizer, max_length=args.max_length)
